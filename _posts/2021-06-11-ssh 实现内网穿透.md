...
- [windows 下 xshell 隧道 Remote(Incoming)模式](https%3A%2F%2Fwww.jianshu.com%2Fp%2F7362479db74f)
- [https://cloud.tencent.com/developer/article/1495849](https://cloud.tencent.com/developer/article/1495849)

# 场景：

![](https://xdo0.github.io/_posts/imgsrc/boxcnETtd2P1bidVAFAC7QhYI6e.png)

![](https://xdo0.github.io/_posts/imgsrc/boxcnfJ1wlurl07PDMeRx6hkaHj.png)

# 隧道/端口转发

**感觉 mobaxterm 这个功能更好用直观些**
...
![](https://xdo0.github.io/_posts/imgsrc/boxcnwJP65GavA2VBtlDcIO3Wyb.png)

1. 打开日志，点击跟踪，勾选协议事件：

![](https://xdo0.github.io/_posts/imgsrc/boxcnXTeuDb369vpRLk4MG9QK2f.png)

1. 点击隧道，点击添加，按如下图填写转移规则

![](https://xdo0.github.io/_posts/imgsrc/boxcnPJUI1olYcZmzeSy1nrelvg.png)

1. 使用主机 C 访问主机 A

经过上面操作，打开新建的会话时会有如下提示

![](https://xdo0.github.io/_posts/imgsrc/boxcnMflwkWpL37ov7CBJ9RjVuh.png)
...
1. 在其它主机 D 可以直接 ssh 主机 C 的 IP（95.49.4.*） + 监听的端口（6634） 访问主机 A
...
![](https://xdo0.github.io/_posts/imgsrc/boxcnZDZcOZJh3iIKuEIFqdFdje.png)
...
Refer：[Linux 下 ssh 反向隧道进行内网穿透](https%3A%2F%2Fwww.jianshu.com%2Fp%2F15c5c1dc40e8)
...
1. 服务器 b 设置代理（可选，如果需要服务器 b 下载的话）
2. 设置服务器 b 的 ssh 代理：
...
1. 先设置通过公钥登录
2. 新建一个脚本 reverse_ssh.sh，注意替换 `Pa` 和第 6 行
...
可能是工位机的 ip 不固定，比如经常切换 wifi 和网线连接

Refer：[SSh 连接失败，Socket error Event: 32 Error: 10053.](https%3A%2F%2Fzhuanlan.zhihu.com%2Fp%2F58336487)
...
[linux_shell 自定义命令 - narjaja - 博客园](https%3A%2F%2Fwww.cnblogs.com%2Fnarjaja%2Fp%2F9231646.html)
...
if [ $1 = 1 ]
then
        netstat -anp | grep ssh|grep "159.226.95.33\|9988" | awk -F ' ' '{len=split($7,arr,"/");for(i=1;i<len;i++){$7=arr[i];print $7}}'| xargs kill
        echo " success "
fi
